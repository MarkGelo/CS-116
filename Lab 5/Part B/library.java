...
class library{
	...
	ArrayList<BookRecord> lib;
	
	library(){
		/* Task 2: Initiliaze vector*/
		lib = new ArrayList<BookRecord>();
	...
	//arg[0]: text file
		...
		while(!option1.equals("Q")){
			...
			/*add code here*/
				...
				for(int i = 0; i < myLib.lib.size(); i++){
					if(d == myLib.lib.get(i).getGenre()){
						libAr[col][count1] = myLib.lib.get(i);
						count1 += 1;
					}else{
					}
				}
			...
			for(int i = 0; i < myLib.lib.size(); i++){
				for(int j = 0; j <myLib.lib.size(); j++){
					if(i != j){
						if(myLib.lib.get(i).equals(myLib.lib.get(j))){
							System.out.println(myLib.lib.get(j));
							myLib.lib.remove(j);
						}
					}
				}
